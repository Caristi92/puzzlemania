    ...
    for pxh in range(Tile.h):
        for pxw in range(Tile.w):
            if tile3.get_at((pxw, pxh)) ==  tile1.get_at((pxw, pxh)):
                ...
                # if there is one pixel that is different it quits, they are not equal
                break
    pixels = Tile.h * Tile.w
    if pixels == uguale: 
        play(choice(["joy", "yes", "ok", "nice", "right"]))
        Puzzle.score += 25
        brighten = 32
            ...
            # altrimenti rimetto il tile nel puzzle 2
                ...
                if Event_listener.drag:
                    ...
                    if event.pos[0] > Puzzle.w // 2 + Puzzle.w + 14:
                        ...
                        # vedo le coordinate 
                        # coord[0] è un numero
                        # coord[1] è l'ascissa 0.... 9
                        # coord[2] + l'ordinata 0 ... 9
                        coord2 = get_coords2(event.pos)
                        # Se non c'è un altro pezzo
                        # coord2[0] contiene il numero del
                        # pezzo del puzzle (da 1 a 100) come sono disposti
                        # es: puzzle2[10][1] ci dà la Surface alla casella 10
                        # tile de terzo quadro
                        # coord2[0] ti dice il numero della casella da 1 a 100
                        # se l'immagine nella casella 10 è uguale all'immagine
                        # nel puzzle originale alla stessa casella 10, allora ok
                        print(coord2)
